...
import { DModulesAction } from "../../scripts/deployment/config/libraries/proxyModules.js";

import { assert } from "chai";

// The ModulesProxy example comes with 8 function selectors
// [runModuleFuncSelectorsAction, loupe, loupe, loupe, loupe, erc165, transferOwnership, owner]
...
describe("Cache bug test", async () => {
	...
	before(async function () {
		...
		let proxyModulesAddress = await deployDProxyModules();
		let dModulesActions = await ethers.getContractAt("DModulesActions", proxyModulesAddress);
		dProxyModulesFuncSelectors = await ethers.getContractAt("DProxyModulesFuncSelectors", proxyModulesAddress);
		const Test1Module = await ethers.getContractFactory("Test1Module");
		test1Module = await Test1Module.deploy();
		...
		if (!receipt.status) {
			throw Error(`ModulesProxy upgrade failed: ${tx.hash}`);
		}
	...
	it("should not exhibit the cache bug", async () => {
		...
		let selectors = await dProxyModulesFuncSelectors.getModuleFuncSelectors(test1Module.address);
