...
/**
 * @title Library for utilising enumerated uint256 sets.
 *
 * @notice Sets have the following properties:
 * values are stored as uint256[] and mapping of array indexes uint256 => uint256
 *
 * - Elements are added, removed, and checked for existence in constant time
 * (O(1)).
 * - Elements are enumerated in O(n). No guarantees are made on the ordering.
 *
 * Include with `using EnumerableUint256Set for EnumerableUint256Set.Uint256Set;`.
 * */
library EnumerableUint256Set {
	 ...
	 */
	function addUint256(Uint256Set storage set, uint256 value) internal returns (bool) {
	 ...
	 */
	function removeUint256(Uint256Set storage set, uint256 value) internal returns (bool) {
		if (contains(set, value)) {
	 ...
	 */
	function get(Uint256Set storage set, uint256 index) internal view returns (uint256) {
